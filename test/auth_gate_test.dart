...
void main() {
  ...
  testWidgets('AuthGate shows LoginScreen when user is signed out', (tester) async {
    final mockAuth = MockFirebaseAuth(signedIn: false);
    final service = AuthService(auth: mockAuth);

    await tester.pumpWidget(
      MaterialApp(home: AuthGate(auth: mockAuth, authService: service)),
    );
  ...
  testWidgets('AuthGate shows HomeScreen when user is signed in', (tester) async {
    final user = MockUser(
      isAnonymous: false,
      uid: 'uid-123',
      email: 'test@test.com',
